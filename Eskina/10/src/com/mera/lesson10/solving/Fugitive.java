package com.mera.lesson10.solving;

import com.mera.lesson10.Cell;
import com.mera.lesson10.Player;

import java.util.*;

import static com.mera.lesson10.Cell.Type.ESCAPE;
...
public class Fugitive {
    ...
    public Fugitive(Cell[][] grid, Cell start, Cell end, Player player) {
        this.player = player;
    ...
    //return true if the exit was found
    public boolean step(Node current) {
        if (isEnd(current)) {
            return true;
        }
        closed.add(current);
        updateNeighbors(current);
        player.setStepNum(player.getStepNum() + 1);
        return false;
    }
