    ...
    public void attackTarget(Creature master){
        ...
        for (int i = 0; i < creatures.length; i++){
            if (creatures[i] == master){
                creatures[i].healthHealing(getDamage());
                if (creatures[i].getHealth() > Parameters.HitPoints.FIRE_WIZARD.getHealth()){
                    creatures[i].setHealth(Parameters.HitPoints.FIRE_WIZARD.getHealth());
                }
                System.out.printf("%s heals himself %d HP. His current health is %s.%n", creatures[i].getName(), getDamage(), creatures[i].getHealth());
                checkCharacterDeath(i);
            }
        }
    }
